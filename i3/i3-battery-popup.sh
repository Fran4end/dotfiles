...
show_notify() {
  ...
  # Ensure DBUS_SESSION_BUS_ADDRESS is set for the script
  export $(dbus-launch)

  # Run the notify-send command with correct display
  DISPLAY=:0 notify-send --urgency=CRITICAL "${1}" ${NOTIFY_OPT}
}
# show_notify() {
#   GNOME_ICON="/usr/share/icons/gnome/scalable/status/battery-low-symbolic.svg"
#   XFCE_ICON="/usr/share/icons/elementary-xfce/status/48/battery-low.png"
